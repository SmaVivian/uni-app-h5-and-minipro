	...
	export default{
		...
		data(){
			return {
				...
				ctx: null,
				isShow: false,
				qrcode: 'https://img.36krcdn.com/20200411/v2_16417a06088947debe0450950f8fc813_img_png' // 二维码
				// qrcode: 'https://huangchunhongzz.gitee.io/imgs/poster/code.png'
		...
		methods:{
			...
			async __init(){
				...
				this.drawText(this.title, ((this.canvasW-hW) / 2),(((this.canvasW-hW) / 2) + hH + 30), 260, 28)
				...
				// ((this.canvasW-hW) / 2) + hH + 90))
				//绘制虚线
				this.drawDashLine(this.ctx,10,(((this.canvasW-hW) / 2) + hH + 80),(this.canvasW-10),(((this.canvasW-hW) / 2) + hH + 80),5)
				//左边实心圆
				this.drawEmptyRound(this.ctx,0,(((this.canvasW-hW) / 2) + hH + 80),10)
				//右边实心圆
				this.drawEmptyRound(this.ctx,this.canvasW,(((this.canvasW-hW) / 2) + hH + 80),10)
				//提示文案
				this.ctx.setFontSize(12);
				this.ctx.setFillStyle('#858585');
				this.ctx.fillText('长按识别图中“文博资讯”小程序',80,(((this.canvasW-hW) / 2) + hH + 120))
				// this.ctx.fillText('vv正在阅读',80,(((this.canvasW-hW) / 2) + hH + 170))
				//底部广告
				...
				this.ctx.drawImage(qrcodeImg.path,20,(((this.canvasW-hW) / 2) + hH + 100), 50, 50)
				//延迟渲染
				setTimeout(()=>{
					this.ctx.draw(true,()=>{
						uni.hideLoading()
